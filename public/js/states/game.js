...
(function() {
  ...
  ToeFu_Clone.Game.prototype.create = function() {
    ...
    this.game.add.tileSprite(0,0,ToeFu_Clone.ASSETS.IMAGE.BG.width,ToeFu_Clone.ASSETS.IMAGE.BG.height, ToeFu_Clone.ASSETS.IMAGE.BG.name);
  ...
  ToeFu_Clone.Game.prototype.update = function() {
    ...
    [this.player_1, this.player_2].forEach(function(player){
      ...
      if(player.body.y > ToeFu_Clone.Game.FLOOR_Y){
        player.body.y = ToeFu_Clone.Game.FLOOR_Y;
  ...
  ToeFu_Clone.Game.prototype.resolve_match = function(victor, loser){
    victor.victory();
    loser.defeat();
  ...
  ToeFu_Clone.Game.prototype.flash = function(message, cb){
  ...
  ToeFu_Clone.Game.prototype.enable_restart_game = function(){
  ...
  ToeFu_Clone.Game.prototype.continue = function() {
    if(this.match_state === MATCH.RESOLVED){
      this.state.start(ToeFu_Clone.STATES.BOOT);
    }
  };
